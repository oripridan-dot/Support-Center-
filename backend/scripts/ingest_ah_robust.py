...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("ah_ingestion.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("AH-Ingestor")

async def ingest_ah():
    scraper = PABrandsScraper()
    
    # Load links
    links_file = "/workspaces/Support-Center-/backend/data/ah_product_links.txt"
    ...
    logger.info(f"Starting ingestion for {len(product_links)} Allen & Heath products")

    async with async_playwright() as p:
        # Try headless=True first with stealth
        ...
        with next(get_session()) as session:
            # Try to find the brand with exact name or containing "Allen"
            brand = session.exec(select(Brand).where(Brand.name == "Allen & Heath")).first()
            if not brand:
                brand = session.exec(select(Brand).where(Brand.name.contains("Allen"))).first()
            
            if not brand:
                logger.error("Brand 'Allen & Heath' not found in database!")
                return
            ...
            for i, url in enumerate(product_links):
                ...
                try:
                    ...
                    # Scrape product page using the shared page
                    try:
