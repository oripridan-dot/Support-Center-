...
class AllenHeathScraper(BaseScraper):
    ...
    async def _extract_media_from_page(self, soup, page_url: str):
        """Extract images, PDFs, and media from page for attachment"""
        try:
            # Find images
            for img in soup.find_all('img'):
                ...
                if img_src:
                    ...
                    if self._is_official_domain(img_url):
                        # Store for later processing
                        pass
            
            # Find PDFs and manuals
            for link in soup.find_all('a', href=True):
                href = link.get('href')
                if href and any(ext in href.lower() for ext in ['.pdf', '.zip', '.exe']):
                    doc_url = urljoin(page_url, href)
                    if self._is_official_domain(doc_url):
                        # Mark for media extraction
                        pass
        except Exception as e:
            logger.debug(f"Error extracting media from {page_url}: {e}")
    ...
    def _is_valid_url(self, url: str) -> bool:
        ...
        # Only keep valid allen-heath.com URLs
...
class AllenHeathIngestionPipeline:
    ...
    async def run(self):
        ...
        logger.info(f"Total URLs to ingest: {len(urls)}")
