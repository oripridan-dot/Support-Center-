...
export default function BrandIngestionBar({ brandName, progress, isCurrent, totalDocuments, targetDocuments }: BrandIngestionBarProps) {
  ...
  if (hasProgress) {
    urlsDiscovered = progress!.urls_discovered;
  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-50">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">
            {status === 'discovering' ? 'Discovering...' : status === 'processing' ? 'Ingesting...' : status === 'complete' ? 'Complete' : 'Coverage'}
          </span>
        </div>
      ...
      <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
        <div 
          className={`h-full transition-all duration-500 ease-out ${status === 'complete' ? 'bg-green-500' : 'bg-blue-500'}`}
          style={{ width: `${percent}%` }}
        />
      </div>
